  ...
  return (
    <div className="flex-1 bg-zinc-900/20 rounded-[3rem] border border-white/5 relative overflow-hidden flex items-center justify-center p-8 lg:p-12 shadow-2xl shadow-black">
      ...
      <div className="absolute inset-0 pointer-events-none z-20 overflow-hidden">
        <div
          className={`absolute w-full h-[1px] transition-all duration-75 ease-linear ${controlMode === 'manual' ? 'bg-indigo-400 shadow-[0_0_20px_rgba(129,140,248,0.7)]' : 'bg-white/40 shadow-[0_0_15px_rgba(255,255,255,0.4)]'}`}
          style={{ top: `${(centroid.y / gridSize) * 100}%` }}
        />
        <div
          className={`absolute h-full w-[1px] transition-all duration-75 ease-linear ${controlMode === 'manual' ? 'bg-indigo-400 shadow-[0_0_20px_rgba(129,140,248,0.7)]' : 'bg-white/40 shadow-[0_0_15px_rgba(255,255,255,0.4)]'}`}
          style={{ left: `${(centroid.x / gridSize) * 100}%` }}
        />
        <div
          className={`absolute w-10 h-10 border-2 rounded-full -translate-x-1/2 -translate-y-1/2 transition-all duration-75 ease-linear flex items-center justify-center ${controlMode === 'manual' ? 'border-indigo-400 scale-125' : 'border-white opacity-50'}`}
          style={{ top: `${(centroid.y / gridSize) * 100}%`, left: `${(centroid.x / gridSize) * 100}%` }}
        >
          <div className={`w-2 h-2 rounded-full animate-ping ${controlMode === 'manual' ? 'bg-indigo-400' : 'bg-white'}`} />
        </div>
      </div>

      {/* Grid */}
      <div
        className="grid gap-[1px] bg-white/5 p-[1px] rounded-lg relative z-10 w-full max-w-[min(100%,70vh)] aspect-square shadow-[0_0_100px_rgba(0,0,0,0.8)] border border-white/10 overflow-hidden"
      ...
      <div className="absolute bottom-10 left-10 flex items-center gap-4 bg-black/80 backdrop-blur-xl px-6 py-3 rounded-full border border-white/10 text-[10px] text-zinc-400 font-black tracking-widest uppercase z-30 shadow-2xl">
        {controlMode === 'auto' ? (
          <><Cpu size={14} className="text-white" /> Matrix Dynamics</>
        ) : (
